 ...
 * is the guild owner, is in the allowlist, or is the bot owner.
 *
 * FIX: Previously isBotOwner bypassed the no-prefix check globally, allowing
 * the bot owner to use no-prefix in ANY server — even those where no-prefix
 * mode was disabled. Now the guild must have no-prefix mode enabled first.
 */
export async function canUseNoPrefix(member) {
    ...
    // Bot owner has access only when no-prefix mode is enabled for this guild
