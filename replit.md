...
- **AI Assistant tool set (20 tools)** — moderation: `ban_member`, `kick_member`, `timeout_member`, `untimeout_member`, `warn_member`, `purge_messages`. Channels: `lock_channel`, `unlock_channel`, `set_slowmode`, `create_channel` (text/voice/category/announcement), `delete_channel`, `rename_channel`, `set_channel_topic`. Roles: `add_role`, `remove_role`, `create_role` (no permission grants — security), `delete_role`, `set_nickname`. Server: `send_announcement` (posts an embed to a channel), `setup_ticket_panel` (configures `ticket_settings` row, auto-creates category + transcript channel if missing, posts the panel button).
