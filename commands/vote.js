...
async function handleCheck(interaction) {
    // Defer immediately — this command does DB queries + an external HTTP call
    // which can easily exceed Discord's 3-second interaction window.
    ...
    if (mutualGuilds.size <= 1) {
        ...
        return interaction.editReply({ embeds: [buildVoteSuccessEmbed(color, voteUrl, coinsEarned, newStreak, newTotal, interaction.guild.name, expiresAt)] });
    }
