...
import { createAIChatCompletion } from "../utils/aiProvider.js";
...
export const data = new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
        sub.setName("summarize")
            .setDescription("Use AI to summarize the conversation in this ticket")
            .addChannelOption((opt) =>
                opt.setName("channel")
                    .setDescription("Ticket channel to summarize (defaults to current channel)")
                    .addChannelTypes(ChannelType.GuildText)
                    .setRequired(false)));
...
async function handleSummarize(interaction) {
    ...
    // Fetch up to 500 messages for summarization
    const messages = await fetchAllMessages(channel, 500);
    if (messages.length === 0) {
        return interaction.editReply({ content: "❌ No messages found in that channel." });
    }
    ...
    let summary;
    try {
        const resp = await createAIChatCompletion({
            maxTokens: 500,
            messages: [
                {
                    role: "system",
                    content:
                        "You are a Discord support ticket summarizer. Given a conversation, write a concise summary covering: (1) the user's issue, (2) what was discussed or attempted, (3) the current status or resolution. Be factual. Plain text, 3–5 sentences max.",
                },
    ...
    if (!summary) {
        return interaction.editReply({ content: "❌ AI summarization failed. Make sure the AI provider is configured." });
    }

    const embed = new EmbedBuilder()
        .setColor(color)
        .setTitle("📋 AI Ticket Summary")
        ...
        .setFooter({ text: "Generated by AI Assistant" })
        .setTimestamp();

    // Post summary in the ticket channel and also reply
