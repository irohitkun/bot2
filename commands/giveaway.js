...
// Max giveaway duration. Long delays are chunked via safeSetTimeout in the
...
export const data = new SlashCommandBuilder()
    ...
    .addStringOption((o) => o.setName("duration").setDescription("Duration e.g. 10m, 1h, 7d (max 30d)").setRequired(true))
...
export async function execute(interaction) {
    ...
    if (sub === "start") {
        ...
        if (!durationMs)
            return interaction.reply({ content: "❌ Invalid duration. Use formats like `10m`, `1h`, `7d` (max 30 days).", flags: 64 });
    ...
    if (sub === "extend") {
        ...
        const extraMs = parseDuration(extraStr);
        if (!extraMs)
            return interaction.reply({ content: "❌ Invalid duration. Use formats like `30m`, `2h`, `5d` (max 30 days).", flags: 64 });
        const [row] = await db.select().from(giveawaysTable).where(and(eq(giveawaysTable.messageId, messageId), eq(giveawaysTable.ended, false)));
        if (!row)
            return interaction.reply({ content: "❌ No active giveaway found with that message ID.", flags: 64 });
        ...
        // Refresh the giveaway message embed
        const channel = await interaction.client.channels.fetch(row.channelId).catch(() => null);
        if (channel) {
            const msg = await channel.messages.fetch(messageId).catch(() => null);
            if (msg) {
                const updatedEmbed = new EmbedBuilder().setColor(0xf1c40f).setTitle("🎉 GIVEAWAY 🎉")
                    .setDescription(`**Prize:** ${row.prize}\n\nReact with 🎉 to enter!\n\n**Ends:** <t:${Math.floor(newEndsAt.getTime() / 1000)}:R>\n**Winners:** ${row.winnersCount}`)
                    .setFooter({ text: `Hosted by ${row.hostTag}` }).setTimestamp(newEndsAt);
                await msg.edit({ embeds: [updatedEmbed] }).catch(() => {});
            }
        }
        // Schedule a new timer; the older one will see the updated endsAt and reschedule itself.
