...
export async function execute(interaction) {
    ...
    if (durationMs > maxMs) return interaction.reply({ content: "❌ Duration cannot exceed 28 days.", flags: 64 });

    const member = await guild.members.fetch(target.id).catch(() => null);
    if (!member) return interaction.reply({ content: "❌ Could not find that member in the server.", flags: 64 });
    if (!member.moderatable) return interaction.reply({ content: "❌ I cannot mute this user — they may have a higher role than me.", flags: 64 });
