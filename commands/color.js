...
export const data = new SlashCommandBuilder()
    .setName("color")
    .setDescription("Preview a color and see its values")
    .addStringOption((opt) =>
        opt.setName("color")
            .setDescription("Hex color (e.g. ff5733 or #ff5733) or RGB (e.g. rgb(255,87,51))")
            .setRequired(true)
    );
...
export async function execute(interaction) {
    ...
    if (!parsed) {
        return interaction.reply({
            content: "❌ Invalid color. Use a hex code like `ff5733` or `#ff5733`, or `rgb(255, 87, 51)`.",
            flags: 64,
        });
    }
