import { SlashCommandBuilder, EmbedBuilder, PermissionFlagsBits } from "discord.js";
  ...
  export async function execute(interaction) {
      ...
      if (!premium) {
          return interaction.reply({ embeds: [premiumDeniedEmbed("No-Prefix Mode")], flags: 64 });
      }
      ...
      if (sub === "list") {
          ...
          const users = rows.filter((r) => r.targetType === "user").map((r) => `<@${r.targetId}>`);
          const roles = rows.filter((r) => r.targetType === "role").map((r) => `<@&${r.targetId}>`);
          const embed = new EmbedBuilder()
              .setColor(0x5865f2)
              ...
              .addFields(
                  { name: "Allowed Users", value: users.length ? users.join("\n") : "None", inline: true },
                  { name: "Allowed Roles", value: roles.length ? roles.join("\n") : "None", inline: true }
              )
              .setTimestamp();
